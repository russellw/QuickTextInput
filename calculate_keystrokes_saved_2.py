...
class ContextualAutocompleteProgram:
    ...
    def build_word_frequency(self):
        ...
        for prefix in self.prefix_dict:
            self.prefix_dict[prefix] = sorted(self.prefix_dict[prefix], key=lambda w: -self.word_freq[w])[:10]
    ...
    def get_predictions(self, word, previous_word):
        ...
        if previous_word in self.bigram_freq:
            ...
            for prefix in set(word[:i] for i in range(1, len(word) + 1)):
                ...
                candidates = sorted(bigram_candidates.keys(), key=lambda w: -bigram_candidates[w])
    ...
    def run(self):
        ...
        print(f"Percentage keystrokes saved: {100 * saved_keystrokes / total_keystrokes:.2f}%")
